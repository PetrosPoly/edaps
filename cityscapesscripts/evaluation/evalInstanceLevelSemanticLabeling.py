...
def setInstanceLabels(args, logger):
    ...
    logger.info('From def setInstanceLabels(args): --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py')
...
def readGTImage(gtImageFileName, args, mapillary_dataloading_style='OURS',
    ...
    if 'mapillary' in dataset_name:
        ...
        if debug:
            logger.info('in debug mode downsample the Cityscapes GT labels to 512 x 1024 : From def readGTImage() --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py')
            img = img.resize((1024, 512), Image.NEAREST)
    else:
        NotImplementedError('No implementation Error --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py')
    return img
...
def filterGtInstances(singleImageInstances,args):
    ...
    # logger.info('From def filterGtInstances(singleImageInstances,args):: --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py')
    # logger.info('args.instLabels: {}'.format(args.instLabels))
    return instanceDict
...
def matchGtWithPreds(predictionList, groundTruthList, gtInstances, args, mapillary_dataloading_style='OURS',
    ...
    matches = {}
    if not args.quiet:
        logger.info("in def matchGtWithPreds() --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py : Matching {} pairs of images...".format(len(predictionList)))
    count = 0
...
def assignGt2Preds(gtInstancesOrig, gtImage, predInfo, args):
    ...
    # logger.info('From def assignGt2Preds(gtInstancesOrig, gtImage, predInfo, args): --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py')
    # logger.info('args.instLabels: {}'.format(args.instLabels))
...
def evaluateMatches(matches, args, logger):
    ...
    logger.info('From def evaluateMatches(matches, args): --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py')
    logger.info('args.instLabels: {}'.format(args.instLabels))
...
def computeAverages(aps,args, logger):
    ...
    logger.info('From def computeAverages(aps,args): --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py')
    logger.info('args.instLabels: {}'.format(args.instLabels))
...
def printResults(avgDict, args, logger):
    ...
    logger.info('From def printResults(avgDict, args): --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py')
    logger.info('args.instLabels: {}'.format(args.instLabels))
...
def prepareJSONDataForResults(avgDict, aps, args, logger):
    ...
    logger.info('From def prepareJSONDataForResults(avgDict, aps, args): --> cityscapesscripts/evaluation/evalInstanceLevelSemanticLabeling.py')
    logger.info('args.instLabels: {}'.format(args.instLabels))
